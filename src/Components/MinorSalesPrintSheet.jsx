...
import { getOwnerOrgName, getOwnerOrgAddress, getOwnerOrgNameRegional, getOwnerOrgMobiles, getOwnerLogoUrl, normalizeImageUrl } from "../utils/ownerConfig";
...
const MinorSalesPrintSheet = forwardRef(function MinorSalesPrintSheet({ active = true, vals = {} }, ref) {
  ...
  const branding = vals?.ownerBranding || {};
  const orgName = String(branding?.orgName || "").trim() || getOwnerOrgName() || "Motera";
  const orgNameRegional = String(branding?.orgNameRegional || "").trim() || getOwnerOrgNameRegional();
  const orgAddress = String(branding?.orgAddress || "").trim() || getOwnerOrgAddress();
  const orgMobiles = Array.isArray(branding?.orgMobiles) && branding.orgMobiles.length
    ? branding.orgMobiles
    : getOwnerOrgMobiles();
  const logoUrl = getOwnerLogoUrl() || normalizeImageUrl(branding?.logoUrl || "") || "";
...
.shop .name-line { font-size: 14.5pt; font-weight: 700; line-height: 1.08; }
.shop .sub { font-size: 9.5pt; margin-top: 1mm; }
.shop .mob { font-size: 9.5pt; margin-top: .8mm; font-weight: 600; }
...
.sign { border-top: 1px dashed #777; padding-top: 3mm; text-align: center; font-size: 10pt; }
      ...
      <div className="wrap">
        <div className="box">
          <div className="hdr">
            {logoUrl ? (
              <img src={logoUrl} alt={orgName} style={{ width: "100%", maxHeight: 92, objectFit: "contain" }} />
            ) : null}
            <div className="shop">
              <div className="name-line">{orgName}{orgNameRegional ? ` | ${orgNameRegional}` : ""}</div>
              {orgAddress ? <div className="sub">{orgAddress}</div> : null}
              {orgMobiles?.length ? <div className="mob">Mob: {orgMobiles.join(" / ")}</div> : null}
            </div>
            <div>
            </div>
          </div>
